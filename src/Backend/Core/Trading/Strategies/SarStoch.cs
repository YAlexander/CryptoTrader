...
namespace Core.Trading.Strategies
{
	public class SarStoch : BaseStrategy
	{
		...
		{
			SarStochOptions options = Options.GetOptions<SarStochOptions>();

			Validate(candles, options);

			List<(ICandle, TradingAdvices)> result = new List<(ICandle, TradingAdvices)>();

			StochItem stoch = candles.Stoch(options?.Stoch ?? 13);
			StochItem stochFast = candles.StochFast(options?.StochFast ?? 13);
			List<decimal?> sar = candles.Sar(options?.SarAccelerationFactor ?? 3);
