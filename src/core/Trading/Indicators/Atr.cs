...
namespace core.Trading.Indicators
{
	public class Atr : BaseIndicator
	{
		...
		public override dynamic Get (IEnumerable<ICandle> source, IIndicatorOptions options = null)
		{
			...
			TicTacTec.TA.Library.Core.RetCode adx = TicTacTec.TA.Library.Core.Atr(0, source.Count() - 1, highs, lows, closes, config.Period, out int outBegIdx, out int outNbElement, atrValues);
